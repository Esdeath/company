...
除 `/api/health/live` 与 `/api/health/ready` 外，所有业务 API 都使用 `/api/v1` 前缀。JSON 字段使用 `snake_case`，时间使用带时区的 ISO 8601；列表使用游标分页。OpenAPI 是前后端契约来源。这两个基础设施端点是唯一不版本化例外，不要求管理员认证，只返回最小状态且不泄露内部信息：`live` 只表示 FastAPI 进程存活，`ready` 只返回能否接收请求的布尔状态，不在响应中列出数据库、目录或任务队列等内部检查细节。
...
发布 API 必须在行锁内重新计算 `content_sha256`，并要求当前 `record id`、`version`、`content_sha256` 与 `nonce` 对应的可信 load-check 为 `passed`，同时验证静态校验为 `clean`；任一状态缺失、失败、过期或绑定旧文件时返回 409。撤回以相同方式锁定记录，把 HTML 原子移出公开路径，更新状态、路径和版本，并追加审计记录。索引状态、文件系统状态和审计结果必须作为一个业务事务完成。
...
首版使用以下固定窗口限流；服务在识别对应 IP 或已认证管理员后、实际执行受限操作或创建任务前检查额度：

- 登录按 IP：15 分钟最多 10 次。
- 公开列表与详情按 IP：每分钟最多 120 次。
- 上传与扫描合并按管理员：每分钟最多 20 次。
- load-check 触发按管理员：每分钟最多 6 次。

可信加载任务同时受容量边界约束：每个快照最多 1 个 `queued` 或 `running` 任务，每个管理员最多 2 个 `queued` 或 `running` 任务，全局最多 4 个并发 headless Chromium，等待队列最多 100 个任务。对同一 `record id`、`version`、`content_sha256` 与 `nonce` 的重复触发返回现有 task id 或已保存的幂等结果，不重复排队或占用新容量；新任务超出限流或遇到队列已满时返回 429，并设置 `Retry-After`。

单机内存计数只允许首版单进程部署；横向扩展必须改用共享限流存储与共享任务队列，不能让各进程分别计算额度或并发数。
